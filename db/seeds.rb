...
private
  ...
  def random_description(opts = {})
    opts[:blocks] ||= 2
    opts[:words] ||= 10
    half = opts[:words] / 2
    f = Forgery(:lorem_ipsum)
    (1..opts[:blocks]).inject([]) { |acc, index|
      acc << "#{f.words(half + rand(opts[:words])).camelize}."
      acc
    }.join("\n")
  end

  def random_html_description(opts = {})
    opts[:blocks] ||= 2
    opts[:words] ||= 10
    half = opts[:words] / 2
    f = Forgery(:lorem_ipsum)
    (1..opts[:blocks]).inject([]) { |acc, index|
      acc << "<p>#{Forgery(:lorem_ipsum).words(half + rand(opts[:words])).camelize}.</p>"
      acc
    }.join("\n")
  end
